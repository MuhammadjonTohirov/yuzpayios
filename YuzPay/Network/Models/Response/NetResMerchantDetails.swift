...
struct NetResMerchantDetails: Codable {
    let id: Int?
    let minAmount, maxAmount: Double?
    let childID: Int?
    let servicePrice, agentCommission, serviceCommission: Double?
    let order: Int?
...
// MARK: - Field
struct MField: Codable, Hashable, Identifiable  {
    let id: Int
    let order: Int?
    let name: String
    let title: String?
    let fieldRequired, readOnly: Bool?
    let fieldType: FieldType? // STRING, REGEXBOX, DATEPOPUP, PHONE, STRING
    ...
    static func == (lhs: MField, rhs: MField) -> Bool {
        return lhs.name == rhs.name
    }
}

// MARK: - ResponseField
struct ResponseField: Codable {
    let fieldName: String
    let label: String?
    let order: Int
}

extension NetResMerchantDetails {
